...
public class TrajectoryPredictor : MonoBehaviour {
    ...
    public LayerMask raycastMask_Object;
    ...
    private void LineDebug(List<Vector3> pointList) {
        if (!debugLineObj || (!reuseLine && debugLineObj)) {
            ...
            debugLine.textureMode = LineTextureMode.Tile;
        ...
        while (!done && iter < iterationLimit) {
            ...
            if (checkForCollision) {
                if (predictionType == predictionMode.Prediction2D) {
                    ...
                    hitInfo2D = Physics2D.Raycast(pos, dir, dist, raycastMask_Object);
                    if(hitInfo2D)
                        hitObject = hitInfo2D.collider.gameObject;
                    if (hit) {
                        if (hit.collider.transform)
                            if (hit.collider.transform != transform) {                                

                                if(hit.collider.gameObject.layer == LayerMask.NameToLayer("Cursor"))
                                {
                                    done = true;
                                }
